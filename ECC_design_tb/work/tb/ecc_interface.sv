...
interface ecc_interface (input logic clk, input logic rst_n);
    ...
    property p_enc_valid_follows_wr;
        @(posedge clk) disable iff (!rst_n)
        (wr_valid && !test_mode_en) |-> ##3 enc_valid_out;  // 3-cycle latency: 2 for GF mult + 1 output reg
    endproperty
    assert property (p_enc_valid_follows_wr) else
        $error("enc_valid_out should follow wr_valid by 3 cycles in normal mode");

    property p_rd_out_follows_rd_in;
        @(posedge clk) disable iff (!rst_n)
        rd_valid |-> ##3 rd_out_valid;  // 3-cycle latency: 2 for GF mult + 1 output reg
    endproperty
    assert property (p_rd_out_follows_rd_in) else
        $error("rd_out_valid should follow rd_valid by 3 cycles");
